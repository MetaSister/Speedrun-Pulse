...
class AllRunsDialog(BaseTableDialog):
    ...
    def handle_all_runs_result(self, data, _):
        ...
        for row, entry in enumerate(all_runs):
            ...
            time_button = QPushButton(SpeedrunPulseApp.format_time(run_obj.get('times', {}).get('primary_t')))
...
class SpeedrunPulseApp(QWidget):
    ...
    def format_time_delta(total_seconds):
        if total_seconds is None or total_seconds <= 0 or not math.isfinite(total_seconds):
            return ""
        return f"(-{SpeedrunPulseApp.format_time(total_seconds)})"
    ...
    def _create_category_item_widget(self, category_data, category_key, game_id, level_id=None, level_name=None, item_type='full_game'):
        ...
        time_button = QPushButton(self.format_time(category_data.get('current_record_time')))
        ...
        stylesheet = THEME.get("stylesheets", {}).get("category_item_widget", "")
        try:
            colors = self.theme.get('colors', {})
            fonts = self.theme.get('fonts', {})
            sizes = self.theme.get('sizes', {})
...
def main():
    ...
    if not (LANGUAGES and THEME):
        ...
        QMessageBox.critical(None, "Initialization Error", f"Critical Error: '{LANGUAGES_FILE}' or '{THEME_FILE}' not found or invalid.")
        sys.exit(1)
    ...
    try:
        ...
        with open(LOG_FILE, 'a', encoding='utf-8') as f:
            ...
            f.write("\\n")
        QMessageBox.critical(None, "Critical Startup Error", f"A critical error occurred. Please check '{LOG_FILE}' for details.\\n\\nError: {e}")
        sys.exit(1)
